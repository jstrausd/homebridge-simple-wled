...
var chacha20poly1305 = __importStar(require("./chacha20poly1305"));
var debug = debug_1.default('encryption');
function fromHex(h) {
    h.replace(/([^0-9a-f])/g, '');
...
function layerDecrypt(packet, count, key, extraInfo) {
    ...
    for (var offset = 0; offset < total;) {
        ...
        if (verifyAndDecrypt(key, nonce, packet.slice(offset + 2, offset + 2 + realDataLength), packet.slice(offset + 2 + realDataLength, offset + 2 + realDataLength + 16), packet.slice(offset, offset + 2), result_Buffer)) {
...
exports.verifyAndDecrypt = verifyAndDecrypt;
...
exports.encryptAndSeal = encryptAndSeal;
